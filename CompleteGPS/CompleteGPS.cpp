...
CompleteGPS::Tokeniser::Tokeniser(char* _str, char _token)
{
    str = _str;
    token = _token;
}
...
bool CompleteGPS::Tokeniser::next(char* out, int len)
{
    uint8_t count = 0;
...
bool CompleteGPS::encode(char c)
{
    buf[pos] = c;
    pos++;
...
bool CompleteGPS::check_checksum()
{
    if (buf[strlen(buf)-5] == '*')
    {
...
uint8_t CompleteGPS::parse_hex(char c)
{
    if (c < '0')
        return 0;
    if (c <= '9')
